...
class GCEncoder(nn.Module):
    def __init__(self, config, weight_init, X_feat=None):
        ...
        self.item_side_info = config.item_side_info
        if self.item_side_info and X_feat is not None:
    ...
    def forward(self, x, edge_index, edge_norm):
        ...
        if self.item_side_info:
            u_features, i_features = self.dense_layer(u_features, i_features, self.X_feat)
        else:
