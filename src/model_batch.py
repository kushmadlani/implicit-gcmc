...
# Main Model
class GAE_Batch(nn.Module):
    def __init__(self, config, weight_init, side_info=None):
        super(GAE_Batch, self).__init__()
        self.gcenc = GCEncoder(config, weight_init, side_info=side_info)
        self.bidec = BiDecoder(config, weight_init)

    def forward(self, x, edge_index, edge_norm, batch_user):
        u_features, i_features = self.gcenc(x, edge_index, edge_norm, batch_user)
...
class GCEncoder(nn.Module):
    def __init__(self, config, weight_init, side_info=None):
        ...
        if self.side:
            self.dense_layer = DenseItemFeatureLayer(config, weight_init)
            self.side_info = side_info
        else:
            self.dense_layer = DenseLayer(config, weight_init)

    def forward(self, x, edge_index, edge_norm, batch_users):
        features = self.rgc_layer(x, edge_index, edge_norm)
        u_features, i_features = self.separate_features(features, batch_users)
    ...
    def separate_features(self, features, batch_users):
        u_features = features[batch_users]
...
class BiDecoder(nn.Module):
    ...
    def forward(self, u_features, i_features):
        ...
        # bilinear form 
        out = torch.chain_matmul(u_features, self.q_matrix, i_features.t())
